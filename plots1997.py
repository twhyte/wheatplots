...
class cwb(object):
    def __init__(self, dataFile = "data.csv"):
        ...
        with open(self.dataFile, 'rt') as csvfile:
            spamreader = csv.DictReader(csvfile, ["Field", "GovernmentMP", "OppositionMP"])
    ...
    def SimpleFarmerRepresentationPlot(self, title="Farmer Representation Arguments - C72/C4", x_label="Argument", y_label="Frequency of Coded Statements"):
        """
        ...
        rects1 = plt.bar(ind, govt, width, color='r')
                            #yerr=menStd,
                            #error_kw=dict(elinewidth=6, ecolor='pink'

        rects2 = plt.bar(ind+width, opp, width, color='b')
                            #yerr=womenStd,
    ...
    def SimpleRegionalRepresentationPlot(self, title="RegionalRepresentation - C72/C4", x_label="Argument", y_label="Frequency of Coded Statements"):
        """
    ...
    def SimpleRationalesPlot(self, category, title="Rationales - C72/C4", x_label="Rationale Type", y_label="Frequency of Coded Statements"):
        """
        Plot of Rationales
        """
    ...
    def StackedRationalesPlot(self, category, title="Rationales for Change - C72/C4", x_label="Rationale Type", y_label="Frequency of Coded Statements"):
        """
        ...
        if category == "Principles":
            group_labels = ["Democracy", "Freedom", "Ideology", "OtherPrinciples","LegalityAndFairness"]
        if category == "ProceduralDemocracy":
            group_labels = ["GeneralGovernmentLegitimacy","ParliamentaryProcedure", "PolicyPlatformCommitment", "TimeForDebate"]
        N = len(group_labels)
        groups = []
        govt = []
        opp = []
        ...
        rects1a = plt.bar(ind, govta, width, color='#ff9999')

        rects1b = plt.bar(ind, govtb, width, color='#ff0000', bottom = govta)

        rects1c = plt.bar(ind, govtc, width, color='#990000', bottom = [sum(n) for n in zip(*[govta, govtb])])

        rects2a = plt.bar(ind+width, oppa, width, color='#99cccc')

        rects2b = plt.bar(ind+width, oppb, width, color='#3399ff', bottom = oppa)

        rects2c = plt.bar(ind+width, oppc, width, color='#000099', bottom = [sum(n) for n in zip(*[oppa, oppb])])
        ...
        if category == "Principles":
            group_labels = ["Democracy", "Freedom", "Ideology","Other Principles","Legality and Fairness"]
        if category == "ProcessLegitimacy":
            group_labels = ["General Government Legitimacy","Parliamentary Procedure", "Policy Platform Commitment","Time For Debate"]
    ...
    def StackedRegionalRepresentationPlot(self, title="Regional Representation Arguments - C72/C4", x_label="Argument Type", y_label="Frequency of Coded Statements"):
        """
        ...
        rects1a = plt.bar(ind, govta, width, color='#ff9999')#99cccc

        rects1b = plt.bar(ind, govtb, width, color='#ff0000', bottom = govta)#3399ff

        rects1c = plt.bar(ind, govtc, width, color='#990000', bottom = [sum(n) for n in zip(*[govta, govtb])])#000099
    ...
    def StackedFarmerRepresentationPlot(self, title="Farmer Representation Arguments - C72/C4", x_label="Argument Type", y_label="Frequency of Coded Statements"):
        """
        Plot of regionalrepresentation, stacked into positive, negative, neutral
        """
    ...
    def SuperSimplePlot(self, title="Overall Argument Frequencies - C72/C4", x_label="Argument Type", y_label="Frequency of Coded Statements"):
        """
        Plot of FarmerRepresentation
        """
        ...
        data = copy.deepcopy(self.data)
        sortedData = []
        group_labels = ['Farmer Representation', 'Regional Representation', 'Economics', 'Process Legitimacy', 'Principles', 'Institutional Value']

        N = len(group_labels)
        groups = []
        govt = [54,43,63,69,96,92]
        opp = [209,148,120,246,302,203]
    ...
    def CombinedWeightedPlot(self, title="Overall Argument Frequencies - Weighted By Days of Debate", x_label="Argument Type", y_label="Frequency of Coded Statements"):
        """
        Plot of FarmerRepresentation
        """
        ...
        data = copy.deepcopy(self.data)
        sortedData = []
        group_labels = ['Farmer Representation', 'Regional Representation', 'Economics', 'Process Legitimacy', 'Value-Driven', 'Worth of Institution']

        N = len(group_labels)
        groups = []
        
        govtRaw = [54,43,63,69,96,92]
        oppRaw = [209,148,120,246,302,203]
        govtConRaw = [416, 202, 268, 89, 278, 174]
        oppLibRaw = [247, 166, 238, 179, 331, 210]
        ...
        for freq in govtRaw:
            govt.append(freq/22.0)
        for freq in oppRaw:
            opp.append(freq/22.0)
        for freq in govtConRaw:
            govtCon.append(freq/10.0)
        for freq in oppLibRaw:
            oppLib.append(freq/10.0)
        ...
        rects3 = plt.bar(ind+(2*width),govtCon, width, color ='white', edgecolor = "black")
        rects4 = plt.bar(ind+(3*width),oppLib, width, color ='white', hatch = "x",edgecolor = "black")
        ...
        plt.title(title, fontsize = 18)
        plt.xticks(ind+2*width, group_labels, fontsize=14)
        plt.xlabel(x_label, fontsize = 16)

        plt.legend( (rects1[0], rects2[0], rects3[0], rects4[0]), ('Government MP - 1997', 'Opposition MP - 1997', "Government MP - 2011", "Opposition MP - 2011") )
    ...
    def constructsPlot(self, title="Standardized Construct Frequencies\nC-72/C-4 Debates (1996-1998)", x_label="Argument Category", y_label="Absolute Frequency of Coded Statements"):
        """
        ...
        for line in data:
            ...
            # Representational Authority measure: adds FarmerRepresentation minus Other and RegionalRepresentation
            ...
            # "GeneralGovernmentLegitimacy" - taken out due to bloc.  Excluded due to the Bloc.
            # example of argument that fits into each
            # whole appendix of account of creation of categories, in appendix for purpose of review
            
            popAuthVars=["TimeForDebate", "ParliamentaryProcedure", "FarmerPlebesciteOnCWB", "Democracy"]
            ...
            ideologyAuthVars=["PolicyPlatformCommitment", "LegalityAndFairness", "Ideology", "Freedom", "ValueOfMonopoly", "HistoryOrOutdated"]
            ...
            # Expert Authority measure: adds Change/Economics, Change/FarmerCostsBenefits, HistoryOrOutdated, OtherChangeInstitutions
            expAuthVars=["Economics", "FarmerCostsAndBenefits"] # Other take out  - causal claims about whether it serves purposes
        ...
        govt.append(repAuthGovt/22.0)
        opp.append(repAuthOpp/22.0)
        govt.append(ideologyAuthGovt/22.0)
        opp.append(ideologyAuthOpp/22.0)
        govt.append(popAuthGovt/22.0)
        opp.append(popAuthOpp/22.0)
        govt.append(expAuthGovt/22.0)
        opp.append(expAuthOpp/22.0)
